          ...
          {eventTypes?.map((et, index) => (
            <AccordionItem className="border-b" value={`${et.duration}m`} key={index}>
              ...
              <AccordionContent className="flex flex-col">
                ...
                <div className="flex flex-row items-center gap-2">
                  <p className={et.is_google_available ? "" : "line-through"}>
                    Google Meet
                  </p>
                  {!et.is_google_available && - <span className="text-xs text-yellow-500">
                    Need action! (connect with google account)
                  </span>}
                </div>
                <div className="flex flex-row items-center gap-2">
                  <p className={et.is_microsoft_available ? "" : "line-through"}>
                    Microsoft Team
                  </p>
                  {!et.is_microsoft_available && - <span className="text-xs text-yellow-500">
                    Need action! (connect with microsoft account)
                  </span>}
                </div>
              </AccordionContent>
            </AccordionItem>
